...
public class ParseEventFn extends DoFn<String, Event> {
  private final Logger LOG = LoggerFactory.getLogger(ParseEventFn.class);
  private final String parseErrorDefaultPrefix = "Parsing error. Unable to parse result item. ";

  @Setup
  public void setup() {
    LOG.debug("Setting up ParseEventFn.");
  }

  @ProcessElement
  public void processElement(@Element String inputElement, OutputReceiver<Event> out) throws Exception {
    try {
      out.output(EventParser.parseEvent(inputElement));
    } catch (Exception e) {
