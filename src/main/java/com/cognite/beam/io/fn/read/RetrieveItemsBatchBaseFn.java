...
/**
 * Base class for retrieving items from CDF and outputting them in batches.
 *
 * Specific resource types (Event, Asset, etc.) extend this class with
 * custom parsing logic.
 */
public abstract class RetrieveItemsBatchBaseFn<T> extends IOBaseFn<Iterable<Item>, List<T>> {
    final ReaderConfig readerConfig;
    final PCollectionView<List<ProjectConfig>> projectConfigView;

    public RetrieveItemsBatchBaseFn(Hints hints,
    ...
    public void processElement(@Element Iterable<Item> items,
        ...
        try {
            ...
            outputReceiver.output(resultsItems);
        } catch (Exception e) {
